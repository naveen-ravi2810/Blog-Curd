...
async def create_blog(blog_details:BlogCreate,session: AsyncSession=Depends(get_session)):
    return await blog_services.create_blog(blog_details=blog_details, session=session)

@blogs_router.put('/blog/{id}')
async def update_blog_id_by_id(id:UUID, blog_update_details: BlogUpdate,session: AsyncSession=Depends(get_session)):
    return await blog_services.update_blog_id_by_id(id, blog_update_details=blog_update_details, session=session )
