...
class BlogServices:
    ...
    async def create_blog(self, blog_details: BlogCreate, session: AsyncSession):
        return await self.repository.create_blog(
            blog_details=blog_details, session=session
        )

    async def update_blog_id_by_id(
        self, id: UUID, session: AsyncSession, blog_update_details: BlogUpdate
    ):
        return await self.repository.update_blog_id_by_id(
            id=id, session=session, blog_update_details=blog_update_details
        )
